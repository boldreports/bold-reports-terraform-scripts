...
resource "aws_ecs_task_definition" "id_web_task" {
  family                   = "${var.app_name}-id-web-task"
  volume {
  ...
  container_definitions    = jsonencode([{
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/bold-identity:${var.id_web_image_tag}"
    portMappings = [
      {
        containerPort = 80
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-id_web_task"
  }
}
resource "aws_ecs_task_definition" "id_ums_task" {
  family                   = "${var.app_name}-id-ums-task"
  volume {
  ...
  container_definitions    = jsonencode([
      ...
      command   = [
        "sh", "-c", 
        "for i in {1..5}; do [ -f /application/app_data/configuration/config.json ] && echo \"Config file found.\" && break || { echo \"Config File not found, waiting for 60 seconds...\"; sleep 60; }; done"
      ]
      mountPoints = [
        {
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/bold-ums:${var.id_ums_image_tag}"
    portMappings = [
      {
        containerPort = 80
        hostPort      = var.launch_type == "FARGATE" ? 80 : 0
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-id_ums_task"
  }
  
}
resource "aws_ecs_task_definition" "id_api_task" {
  family                   = "${var.app_name}-id-api-task"
  volume {
  ...
  container_definitions    = jsonencode([
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/bold-idp-api:${var.id_api_image_tag}"
    portMappings = [
      {
        containerPort = 80
        hostPort      = var.launch_type == "FARGATE" ? 80 : 0
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-id_api_task"
  }
  
}
resource "aws_ecs_task_definition" "reports_web_task" {
  family                   = "${var.app_name}-reports-web-task"
  volume {
  ...
  container_definitions    = jsonencode([
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/boldreports-server:${var.reports_web_image_tag}"
    portMappings = [
      {
        containerPort = 80
        hostPort      = var.launch_type == "FARGATE" ? 80 : 0
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-reports_web_task"
  }
  
}
resource "aws_ecs_task_definition" "reports_api_task" {
  family                   = "${var.app_name}-reports-api-task"
  volume {
  ...
  container_definitions    = jsonencode([
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/boldreports-server-api:${var.reports_api_image_tag}"
    portMappings = [
      {
        containerPort = 80
        hostPort      = var.launch_type == "FARGATE" ? 80 : 0
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-reports_api_task"
  }
  
}
resource "aws_ecs_task_definition" "reports_jobs_task" {
  family                   = "${var.app_name}-reports-jobs-task"
  volume {
  ...
  container_definitions    = jsonencode([
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/boldreports-server-jobs:${var.reports_jobs_image_tag}"
    portMappings = [
      {
        containerPort = 80
        hostPort      = var.launch_type == "FARGATE" ? 80 : 0
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-reports_jobs_task"
  }
  
}
resource "aws_ecs_task_definition" "reports_dataservice_task" {
  family                   = "${var.app_name}-reports-dataservice-task"
  volume {
  ...
  container_definitions    = jsonencode([
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/boldreports-designer:${var.reports_dataservice_image_tag}"
    portMappings = [
      {
        containerPort = 80
        hostPort      = var.launch_type == "FARGATE" ? 80 : 0
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-reports_dataservice_task"
  }
  
}
resource "aws_ecs_task_definition" "reports_viewer_task" {
  family                   = "${var.app_name}-reports-viewer-task"
  volume {
  ...
  container_definitions    = jsonencode([
    ...
    image     = "us-docker.pkg.dev/boldreports/v8-1-1/boldreports-viewer:${var.reports_viewer_image_tag}"
    portMappings = [
      {
        containerPort = 80
        hostPort      = var.launch_type == "FARGATE" ? 80 : 0
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-reports_viewer_task"
  }
  
}
resource "aws_ecs_task_definition" "bold_etl_task" {
  family                   = "${var.app_name}-bold-etl-task"
  volume {
  ...
  memory                   = var.task_memory
  tags = {
    Name = "${var.app_name}-bold_etl_task"
  }
...
resource "aws_ecs_service" "id_web_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-id-web-service"
...
resource "aws_ecs_service" "id_ums_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-id-ums-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  ...
  depends_on = [aws_lb_target_group.id_ums_tg,aws_ecs_service.id_web_service_ec2]
}
resource "aws_ecs_service" "id_api_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-id-api-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  ...
  depends_on = [aws_lb_target_group.id_api_tg,aws_ecs_service.id_web_service_ec2]
}
resource "aws_ecs_service" "reports_web_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-reports-web-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  ...
  depends_on = [aws_lb_target_group.reports_web_tg,aws_ecs_service.id_web_service_ec2]
}
resource "aws_ecs_service" "reports_api_service_ec2" {
  ...
  depends_on = [aws_lb_target_group.reports_api_tg,aws_ecs_service.id_web_service_ec2]
}
resource "aws_ecs_service" "reports_jobs_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-reports-jobs-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  ...
  depends_on = [aws_lb_target_group.reports_jobs_tg,aws_ecs_service.id_web_service_ec2]
}
resource "aws_ecs_service" "reports_dataservice_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-reports-dataservice-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  ...
  depends_on = [aws_lb_target_group.reports_dataservice_tg,aws_ecs_service.id_web_service_ec2]
}
resource "aws_ecs_service" "reports_viewer_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-reports-viewer-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  ...
  depends_on = [aws_lb_target_group.reports_viewer_tg,aws_ecs_service.id_web_service_ec2]
}
resource "aws_ecs_service" "bold_etl_service_ec2" {
  count = var.launch_type == "EC2" ? 1 : 0
  name            = "${var.app_name}-bold-etl-service"
  cluster         = aws_ecs_cluster.ecs_cluster.id
  ...
  depends_on = [aws_lb_target_group.bold_etl_tg,aws_ecs_service.id_web_service_ec2]
}
...
resource "aws_ecs_service" "id_ums_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.id_ums_tg,aws_ecs_service.id_web_service_fargate]
}
resource "aws_ecs_service" "id_api_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.id_api_tg,aws_ecs_service.id_web_service_fargate]
}
resource "aws_ecs_service" "reports_web_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.reports_web_tg,aws_ecs_service.id_web_service_fargate]
}
resource "aws_ecs_service" "reports_api_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.reports_api_tg,aws_ecs_service.id_web_service_fargate]
}
resource "aws_ecs_service" "reports_jobs_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.reports_jobs_tg,aws_ecs_service.id_web_service_fargate]
}
resource "aws_ecs_service" "reports_dataservice_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.reports_dataservice_tg,aws_ecs_service.id_web_service_fargate]
}
resource "aws_ecs_service" "reports_viewer_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.reports_viewer_tg,aws_ecs_service.id_web_service_fargate]
}
resource "aws_ecs_service" "bold_etl_service_fargate" {
  ...
  depends_on = [aws_lb_target_group.bold_etl_tg,aws_ecs_service.id_web_service_fargate]
}
...
resource "aws_lb_target_group" "reports_web_tg" {
  name        = "${var.app_name}-reports-web-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.ecs_vpc.id
  target_type = var.launch_type == "EC2" ? "instance" : "ip"
...
resource "aws_lb_target_group" "reports_api_tg" {
  name        = "${var.app_name}-reports-api-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.ecs_vpc.id
  target_type = var.launch_type == "EC2" ? "instance" : "ip"
...
resource "aws_lb_target_group" "reports_jobs_tg" {
  name        = "${var.app_name}-reports-jobs-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.ecs_vpc.id
  target_type = var.launch_type == "EC2" ? "instance" : "ip"
...
resource "aws_lb_target_group" "reports_dataservice_tg" {
  name        = "${var.app_name}-reports-dataservice-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.ecs_vpc.id
  target_type = var.launch_type == "EC2" ? "instance" : "ip"
...
resource "aws_lb_target_group" "reports_viewer_tg" {
  name        = "${var.app_name}-reports-viewer-tg"
  port        = 80
  protocol    = "HTTP"
  vpc_id      = aws_vpc.ecs_vpc.id
  target_type = var.launch_type == "EC2" ? "instance" : "ip"
